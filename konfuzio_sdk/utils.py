...
def get_paragraphs_by_line_space(
    ...
    :param line_height_ratio: Ratio of the result of median of the distance between lines as threshold
    ...
    for page_char_bboxes in pages_char_bboxes:
        ...
        for bbox in sorted(page_char_bboxes, key=lambda x: x['char_index']):
            if not page_lines or page_lines[-1]['line_index'] == bbox['line_index']:
                line_bboxes.append(bbox)
            else:
                page_lines.append(line_bboxes)
                line_bboxes = [bbox]
        page_lines.append(line_bboxes)
        # page_line_bboxes.append(page_lines)
