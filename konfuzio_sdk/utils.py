...
# TODO Unify get_bbox and get_bboxes (#9349)
def get_bboxes(bbox, start_offset, end_offset) -> List:
    """
    Get a bbox per line for offset_string.

    Similar to `get_bbox`, however gets a list of bboxes that span across multiple lines of selected text.
    ...
    # get every character bbox in the document between the start and end offset
    char_bboxes = [bbox[str(char_bbox_id)] for char_bbox_id in range(start_offset, end_offset)
                   if str(char_bbox_id) in bbox]

    # exit early if no bboxes are found between the start/end offset
    if len(char_bboxes) < 1:
        return []
    ...
    if all('line_number' in char_bbox.keys() for char_bbox in char_bboxes):
    ...
    for line_number, line_char_bboxes in itertools.groupby(char_bboxes, lambda x: x[group_by]):
        ...
        for char_bbox in line_char_bboxes:
            x0 = min(char_bbox['x0'], x0)
            top = min(char_bbox['top'], top)
            y0 = min(char_bbox['y0'], y0)
