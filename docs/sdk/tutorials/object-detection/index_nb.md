...
As an alternative you can install and run the code locally or on an environment of your choice.

### Introduction

This tutorial provides a step by step guide on how to train an object detection model. 🕵 \
The focus here is on Document structures but you can use the code to train on a datasets of different domains as well. 
...
Here is an example of what the model will be capable of after training. The color cyan stands for detected checked boxes and the color green for detected empty boxes, while the number indicated the detection confidence. 😃
...
Once you have your data ready, you need to adapt the following lines, so that the paths to your dataset is defined correctly.
...
And as a last step we instantiate the dataloader. Pay attention to the `min_samples` with value of `512`. This parameter forces the training, to use at least the `min_samples` number of images for each epoch. This is usefull for smaller datasets as we use here. 
...
```python tags=["skip-execution", "skip-nbeval"]
...
<!-- ```python tags=["skip-nbeval", "skip-execution"]
...
<!-- > **Important**❗ \
> If you want to use the model commercially, you need to ensure that the training happens without pretrained weights from `super-gradients`. This is the case if you set `pretrained_weights=None` during instantiation of the super-gradients model. -->
...
Then we get the path to the saved checkpoint, which was saved during the training step.
...
**Dependency installation**
```bash tags=["skip-execution", "skip-nbeval"]
pip install -q super-gradients
pip install -q pycocotools
pip install -q onnx
pip install -q onnxruntime
