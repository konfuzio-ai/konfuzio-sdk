...
class TestRegexTokenizer(unittest.TestCase):
    ...
    def test_evaluate_category(self):
        ...
        result, _ = self.tokenizer.evaluate_category(self.category)
        assert result.is_correct.sum() == 1
        assert result.is_found_by_tokenizer.sum() == 1
...
class TestRegexMatcherTokenizer(TestTemplateRegexTokenizer):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.document_test = Document(project=cls.project, category=cls.category, text="Hi all,", dataset_status=3)
        annotation_set_test = AnnotationSet(id_=4, document=cls.document_test, label_set=cls.label_set)
        cls.span_test = Span(start_offset=3, end_offset=6)
        _ = Annotation(
            id_=6,
            document=cls.document_test,
            is_correct=True,
            annotation_set=annotation_set_test,
            label=cls.label,
            label_set=cls.label_set,
            spans=[cls.span_test],
        )
        ...
        annotation_set_test_a = AnnotationSet(id_=8, document=cls.document_test_a, label_set=cls.label_set)
    ...
    def test_evaluate_category(self):
        ...
        result, _ = self.tokenizer.evaluate_category(category=self.category)
        assert result.is_found_by_tokenizer.sum() == 2

    def test_evaluate_project(self):
        """Test evaluate_project method."""
        self.tokenizer.reset_processing_steps()
        result, _ = self.tokenizer.evaluate_project(project=self.project)
