    ...
    def test_04_save_model(self):
        ...
        with pytest.raises(MemoryError):
            self.pipeline.pipeline_path = self.pipeline.save(
                model_type="extraction_ai", include_konfuzio=False, max_ram="5MB"
            )

        self.project._max_ram = "5MB"
        with pytest.raises(MemoryError):
            self.pipeline.pipeline_path = self.pipeline.save(model_type="extraction_ai", include_konfuzio=False)
        self.project._max_ram = None

        self.pipeline.pipeline_path = self.pipeline.save(
            model_type="extraction_ai", output_dir=self.project.model_folder, include_konfuzio=False
        )
