...
if EXTRAS_INSTALLED in ['all', 'extraction']:
    ...
    class TestAddExtractionAsAnnotation(unittest.TestCase):
        ...
        def test_add_extraction_to_empty_document(self):
            ...
            with pytest.raises(
                NotImplementedError, match='does not have a correspondence in the text of Virtual Document'
            ):
                RFExtractionAI().add_extractions_as_annotations(
                    extractions=self.extraction_df,
                    document=document,
    ...
    class TestExtractionToDocument(unittest.TestCase):
        ...
        def test_extraction_result_for_label_set_with_single_annotation_set(self):
            ...
            extraction_result = {
                'LabelSetName': {'LabelName': pd.DataFrame(data=[self.extraction_1, self.extraction_2])}
            }
            virtual_doc = self.pipeline.extraction_result_to_document(
                self.sample_document, extraction_result=extraction_result
            )
            ...
            assert (
                annotation_1.label_set == annotation_2.label_set == self.project.get_label_set_by_name('LabelSetName')
            )
    ...
    def test_no_running():
        """Test that no tests for this module are run except this."""
        import importlib
